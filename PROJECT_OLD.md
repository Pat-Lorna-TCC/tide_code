...
Extension: packaged distribution of skills (and optional UI surfaces) with a manifest + permissions.
...
21) Remaining Small Open Questions
Do we allow extensions to add UI panels in v0.x, or are extensions engine-only initially (skills/tools only)?

skills tools only for now
...
3) Update §19 milestone M6 to reflect “no UI”
M6 (next): Pi parity layer + extensions
...
4) Add this line to §20 “Locked Decisions”
Extensions: skills/tools only (engine-only) for v0.x; no UI extension surfaces
5) Update §21 (Remaining Open Questions)
Replace §21 with:

21) Remaining Small Open Questions
(None locked right now; next decisions will come from scaffold implementation details like exact manifest schema versioning and permission prompts.)

If you want, I can now rewrite the header/version line as Spec v0.7 and re-emit the full .tide/PROJECT.md as one clean document (no patch-style edits), but the above is the complete content change implied by “skills/tools only for now.”